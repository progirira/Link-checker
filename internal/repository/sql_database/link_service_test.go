...
func TestDeleteTag(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err = db.QueryRow(ctx, `INSERT INTO tags (name) VALUES ($1) 
									ON CONFLICT (name) DO UPDATE SET name = EXCLUDED.name 
									RETURNING id`, tag).Scan(&tagID)
			assert.NoError(t, err)
			assert.Greater(t, tagID, int64(0))

			linkID := int64(1234567)
			tgID := int64(1234567)

			var userID int64

			err = db.QueryRow(ctx, `INSERT INTO users (telegram_id) VALUES ($1) 
									ON CONFLICT (telegram_id) DO UPDATE SET telegram_id = $1 
									RETURNING id`, tgID).Scan(&userID)
			assert.NoError(t, err)

			_, err = db.Exec(ctx, "INSERT INTO links (id, url) VALUES ($1, $2) ON CONFLICT DO NOTHING", linkID, "https://example.com")
			require.NoError(t, err)

			_, err = db.Exec(ctx, `INSERT INTO link_tags (link_id, tag_id, user_id) 
						VALUES ($1, $2, (SELECT id FROM users WHERE telegram_id = $3)) 
						ON CONFLICT DO NOTHING`, linkID, tagID, tgID)
			assert.NoError(t, err)

			err = svc.DeleteTag(ctx, tgID, tag)
			assert.NoError(t, err)

			var count int

			err = db.QueryRow(ctx, "SELECT COUNT(*) FROM link_tags WHERE tag_id = $1", tagID).Scan(&count)
			assert.NoError(t, err)
			assert.Equal(t, 0, count)
		})
	}
